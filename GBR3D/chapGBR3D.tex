...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Campagnes d'observations et simulations numériques à Gibraltar/ Observatons and siulatons of processes in the Strait of Gibraltar(avant ou après partie process etc.)}
...
\begin{figure}[!h]
        ...
        \caption{Ajouter point Tarifa, point ubar...}
\end{figure}
...
\subsubsection{Matching Pursuit(retirer?pas au point)}
...
hydraulic control shape can be two ways during outflow. in spring tide at first resembles the intermediate case//at first the hyd jump forms at the top of the sill, but as the tidal barotropic current strenghten the hydraulic control on the atl layer expends west, but as barotropic currents increase jump is swept wesward as the area of supercritical atl layer extends over the sill.
...
%\end{subfigure}
 \caption {Divergence of surface current and froude of atl layer superior to 1 (changer crois en noir et plus grandes...)}
\end{figure}
...
In those simulation, Even when no hyd jump can see a signal propagating in neap tide

((The , using matching pursuit, see that the waves of the train are resolved enough to fit the scenate(?) profile of solitary waves. 
...
%\end{subfigure}
 \caption {Vorticity of surface current (upper row) and divergence of surface current (lower row)}
\end{figure}


Solitary waves are generated at each tidal cycle at CS, except when there is no . However, even at this point the tide passing through the strait and through tarifa gives a signal at the surface. 

Control at Tarifa occurs at the maximum of inflow, but also when the solitary wave as just passed through this area. This twho events are not necessarily synchronized (at the same time), 
...
Each time solitary waves exits the strait at east side, generation of a positive vorticity / cyclonic eddy at (nom cap...) . The wave of the next tidal cycle gets diffracted on this eddy, creating interference in the train of solitary waves
...
% \centering
 ...
 \caption {std(Q) in last 30 minutes before 14H of simulation time in spring tide simulation, and trace of contour Q of SVD (remettre même palette de couleur que fig controles hyd)}
\end{figure}

\begin{figure}[!h]
% \centering
 \includegraphics[width=\textwidth]{./GBR3D/TS_coupesCso14h.png}
 \caption {T,S diagrams, inset on med outflow area (combiner avec figures indique où coupes...)}
\end{figure}
