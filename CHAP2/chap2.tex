...
\end{equation}
where $\zeta$ is the free-surface anomaly in the vicinity of the geoid and subscribe $H$ indicates that only the horizontal component is considered. Assuming then \color{red}no-slip condition and \color{black}that ocean water cannot penetrate the ocean bottom (at depth $z=-H$):
\begin{equation}
 \displaystyle
 \label{NS_BC0}
...
For a linear approximation of the equation of state, a simple evolution equation of $\rho$ can be obtained as a combination of Equations \noparref{NS_c} and \noparref{NS_d} leading to:\color{red} (eq utilisée pour développement BPE (et ailleurs?) XC : incompressible faut le complet... (mettre els deux) (combinaison des deux équations faut le mettre complet))\color{black}
\begin{equation}
\displaystyle
\frac{d \rho}{d t}=
...
\end{alignat}
\end{subequations}
This splitting conserves basically the formulation of the horizontal momentum equations proposed in \cite{shchepetkin_regional_2005}: the length-scales of the processes and the fast-mode forcing are yet obviously different but the filtering procedure $\ll.\gg$ is the "flat" filter proposed by \cite{shchepetkin_regional_2005}\color{red}in figure 15? (reformuler la phrase pour plus claire????)\color{black}. $[[.]]$ notation indicates the extrapolation in time of the slow-kernel terms to be used at the fast-kernel RHS (see \S \noparref{TimeSplit}).\\
