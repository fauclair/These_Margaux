...
Due to considerations of the representation of bathymetric features, terrain-following coordinates, or S-coordinates, can be choosen for the vertical discretisation. They are generally defined based on "generalized surfaces" $s=cte$ with s given by:
\begin{equation}
 \displaystyle
 s=s(x,y,z,t)
\end{equation}
...
Using coordinate transformation presented in (annexe?), the vertical velocity $w \equiv v_z$ is decomposed as :
\begin{subequations}
  \begin{alignat}{2}
  \displaystyle 
...
(mettre evol densité forme 'simple' en coord s)


\subsection{Conservation of an extensive quantity in a free-surface ocean}


We want to specify the conservation of an extensive property defined as $\iiint_{\mathcal{V}_M(t)} \rho A d\tau$, in a region of the ocean with a varying free-surface, ie. an area of fluid with a varying volume.


The evolution of this quantity.

The Reynolds theorem presented in annexe... treats the case of generalized coordinates for which (donne formulation). In Cartesian coordinates se passe si.... The expression is also easily found in s-coordinates since boundaries remain independant through time.

Can also recover the s-coordinate case from Reynolds transport theorem and the fact that ...vanishes

The choice of s-coordinates makes it easier to put this...

To make evolution of such a quantity, use the Reynolds (transport?) theorem.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Coordinate transformation(formulaire en annexe?)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
